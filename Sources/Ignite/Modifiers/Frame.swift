...
struct FrameModifier: HTMLModifier {
    ...
    func body(content: some HTML) -> any HTML {
        ...
        if let width {
            modified = modified.style(.init(.width, value: width.stringValue))
        }

        if let minWidth {
            modified = modified.style(.init(.minWidth, value: minWidth.stringValue))
        }

        if let maxWidth {
            modified = modified.style(.init(.maxWidth, value: maxWidth.stringValue))
        }

        if let height {
            modified = modified.style(.init(.height, value: height.stringValue))
        }

        if let minHeight {
            modified = modified.style(.init(.minHeight, value: minHeight.stringValue))
        }

        if let maxHeight {
            modified = modified.style(.init(.maxHeight, value: maxHeight.stringValue))
        }
