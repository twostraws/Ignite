...
struct LineSpacingModifier: HTMLModifier {
    ...
    func body(content: some HTML) -> any HTML {
        if content.body.isComposite {
            if let customHeight {
                content.containerStyle(.init(.lineHeight, value: NumberFormatter.format(customHeight)))
            } else if let presetHeight {
                content.containerClass("lh-\(presetHeight.rawValue)")
            }
        } else {
            if let customHeight {
                content.style(.init(.lineHeight, value: NumberFormatter.format(customHeight)))
            } else if let presetHeight {
