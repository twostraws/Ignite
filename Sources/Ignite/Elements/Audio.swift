...
public struct Audio: BlockElement, InlineElement, LazyLoadable {
    ...
    public var columnWidth = ColumnWidth.automatic
        ...
        /// The name of the audio to display. This should be specified relative to the
        /// root of your site, e.g. /audio/bark.mp3.
    var files: [String]?
    
        /// Creates an `Audio` instance from the name of a file contained in your site's assets. This should be specified relative to the root of your site, e.g. /audio/bark.mp3.
        /// - Parameters:
...
extension Audio {
        ...
        /// Each case in the enum represents a different audio format, and the raw value of each case is the MIME type associated with that format.
    public enum AudioType: String {
        ...
        case aac = "audio/aac"
            ...
            /// Standard audio format used by Apple. It stores sound data for personal computers and other electronic audio devices.
        case aif = "audio/aiff"
        ...
        case flac = "audio/flac"
            ...
            /// A playful or proprietary audio format typically used for making fun or funky sounds.
    ...
    public func audioTypes(for filename: String) -> AudioType? {
        switch filename {
            ...
            case let name where name.contains(".aif"):
                return .aif
            case let name where name.contains(".au"), let name where name.contains(".snd"):
                return .au
            case let name where name.contains(".flac"):
                return .flac
            case let name where name.contains(".funk"), let name where name.contains(".my"), let name where name.contains(".pfunk"):
                return .funk
            case let name where name.contains(".gsd"), let name where name.contains(".gsm"):
                return .gsd
            case let name where name.contains(".it"):
                return .it
            case let name where name.contains(".jam"):
                return .jam
            case let name where name.contains(".kar"), let name where name.contains(".mid"):
                return .kar
            case let name where name.contains(".la"), let name where name.contains(".lma"):
                return .la
            ...
            case let name where name.contains(".mod"):
                return .mod
            case let name where name.contains(".mp2"), let name where name.contains(".m2a"):
                return .mp2
            ...
            case let name where name.contains(".qcp"):
                return .qcp
            case let name where name.contains(".ra"), let name where name.contains(".ram"), let name where name.contains(".rm"), let name where name.contains(".rmm"), let name where name.contains(".rmp"):
                return .ra
            case let name where name.contains(".raPlugin"), let name where name.contains(".rmpPlugin"), let name where name.contains(".rpm"):
                return .raPlugin
            ...
            case let name where name.contains(".sndX"):
                return .sndX
            case let name where name.contains(".tsi"), let name where name.contains(".tsp"):
                return .tsi
            case let name where name.contains(".voc"):
                return .voc
            case let name where name.contains(".vox"):
                return .vox
            case let name where name.contains(".vqe"), let name where name.contains(".vql"):
                return .vqe
