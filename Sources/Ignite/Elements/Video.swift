...
public struct Video: BlockElement, InlineElement, LazyLoadable {
    ...
    var files: [String]?
        ...
        /// Creates one or multiple `Video` instance from the name of a files contained in your site's assets. This should be specified relative to the root of your site, e.g. /video/outforwalk.mp4.
        /// - Parameters:
        ///   - names: The filenames of your video relative to the root of your site. e.g. /video/outforwalk.mp4.
...
extension Video {
        /// `VideoType` is an enumeration that defines a list of video file types.
        /// Each case in the enum represents a different video format, and the raw value of each case is the MIME type associated with that format.
    public enum VideoType: String {
        ...
        case isvideo = "video/x-isvideo"
            ...
            /// A modern container format capable of holding an unlimited number of video, audio, picture, or subtitle tracks in one file.
        case matroska = "video/x-matroska"
        
            /// - motionJpeg: Motion JPEG Format
            /// A video format where each video frame is separately compressed as a JPEG image.
    ...
    public func videoType(for filename: String) -> VideoType? {
        switch filename {
            ...
            case let name where name.contains(".gl"):
                return .gl
            case let name where name.contains(".h264"), let name where name.contains(".mp4"):
                return .h264
            case let name where name.contains(".h265"), let name where name.contains(".hevc"):
                return .h265
            case let name where name.contains(".isvideo"):
                return .isvideo
            case let name where name.contains(".matroska"), let name where name.contains(".mkv"):
                return .matroska
            ...
            case let name where name.contains(".ogg"):
                return .ogg
            case let name where name.contains(".quicktime"), let name where name.contains(".mov"):
                return .quicktime
