...
class OpacityTests: IgniteTestSuite {
    @Test("Text Opacity", .publishingContext(), arguments: ["This is a test", "Another test"])
    ...
    @Test("Image Opacity", .publishingContext(), arguments: [(path: "/images/example.jpg", description: "Example image")])
    ...
    @Test("Checks that the opacity value is correctly formatted", .publishingContext(), arguments: zip(
    ...
    @Test("Checks that full opacity is not rendered", .publishingContext())
